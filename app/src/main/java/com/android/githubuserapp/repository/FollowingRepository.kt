...
class FollowingRepository(private val apiService: ApiService) {

    suspend fun getFollowingUser(login: String): AppResult<List<User>> {
        return try {
            val response = apiService.getFollowingUser(login)
